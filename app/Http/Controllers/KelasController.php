...
class KelasController extends Controller
{
    ...
    function memberkelas(Request $req,$id){
      $user = Auth::user()->nim_nip;
      if(Auth::user()->hak_akses=="Mahasiswa"){
        $cek = DB::select("SELECT*FROM kelas,members WHERE kelas.id = members.id_kelas AND members.id_kelas = '$id' AND members.nim_nip_user = '$user'");
        if($cek == null){
          ...
          if($req->cari){
            $data['member'] = DB::select("SELECT members.id, nama_user,poin,nim_nip,level,email, profile_photo_path FROM members,users WHERE members.nim_nip_user = users.nim_nip AND members.id_kelas='$id' AND users.nama_user LIKE '%$req->cari%' OR users.nim_nip LIKE '%$req->cari%' ORDER BY members.poin DESC");
          }else {
              $data['member'] = DB::select("SELECT members.id, nama_user,poin,nim_nip,level,email, profile_photo_path FROM members,users WHERE members.nim_nip_user = users.nim_nip AND members.id_kelas='$id' ORDER BY members.poin DESC");
          }

          $data['totalposts'] = count(DB::select("SELECT*FROM tugas WHERE id_kelas ='$id'")) + count(DB::select("SELECT*FROM challenges WHERE id_kelas='$id'")) + count(Materi::where('id_kelas',$id)->get()->sortByDesc('created_at'));
          $data['kelas'] = DB::select("SELECT kelas.id, cover_kelas, kode_mk, nama_mk, nama_user,kode_kelas,komposisi_eas, komposisi_ets, komposisi_quis, komposisi_tugas, profile_photo_path FROM kelas,users WHERE kelas.id_dosen = users.nim_nip AND kelas.id = $id");
          $data['classinfo'] = Kelas::where('id',$id)->get();
          $data['totalmember'] = count($data['member']);
          return view('page.memberkelas',$data);
        }
      }else {
        if($req->cari){
          $data['member'] = DB::select("SELECT members.id, nama_user,poin,nim_nip,level,email, profile_photo_path FROM members,users WHERE members.nim_nip_user = users.nim_nip AND members.id_kelas='$id' AND users.nama_user LIKE '%$req->cari%' OR users.nim_nip LIKE '%$req->cari%' ORDER BY members.poin DESC");
    ...
    function berinilai(Request $req){
      ...
      $batasexp = 100 + ($levelawaluser * 100);
      $expawaluser  = $expawaluser+$nilai;
      while(true){
        if($expawaluser > $batasexp){
          $levelawaluser = $levelawaluser +1;
          $expawaluser = $expawaluser - $batasexp ;
        }else {
    ...
    function buattugas(Request $req){
      ...
      // $datamemberusers = DB::select("SELECT*FROM members,users WHERE members.nim_nip_user = users.nim_nip AND members.id_kelas ='$req->id_kelas'");
      // $datadosenkelas = DB::select("SELECT*FROM kelas,users WHERE kelas.id_dosen = users.nim_nip AND kelas.id ='$req->id_kelas' ");
      // date_default_timezone_set("Asia/Makassar");
      // $tgltoday = date("l, d-m-Y: h:i:sa");
      // foreach($datadosenkelas as $dtdosen){
      //   $data = array(
      //     'nama_user' => $dtdosen->nama_user,
      //     'nama_mk' => $dtdosen->nama_mk,
      //     'tipe' => "Tugas",
      //     'judul' => $req->judul_tugas,
      //     'deadline_jam' => $req->deadline_jam,
      //     'deadline_tanggal' => $req->deadline_tanggal,
      //     'tanggalhariini' => $tgltoday,
      //   );
      //
      // }
      // $notif = "Funlearning Today ".$tgltoday;
      // foreach($datamemberusers as $detail){
      //   Mail::send('mailnotiftugas',$data, function ($mail) use($detail,$notif){
      //     $mail->to($detail->email, 'no-reply')
      //       ->subject("$notif");
      //     //$mail->from('funlearningitk@gmail.com','Funlearning ITK');
      //   });
      //
      // }
    ...
    function buatmateri(Request $req){
      ...
      // $notif = "Funlearning Today ".$tgltoday;
      // foreach($datamemberusers as $detail){
      //   Mail::send('mailnotif',$data, function ($mail) use($detail,$notif){
      //     $mail->to($detail->email, 'no-reply')
      //       ->subject("$notif");
      //     //$mail->from('funlearningitk@gmail.com','Funlearning ITK');
      //   });
    ...
    function jawabtugas(Request $req){
      ...
      // $idusr = Auth::user()->nim_nip;
      //
      //
      // $datamemberusers = DB::select("SELECT*FROM members,users WHERE members.nim_nip_user = users.nim_nip AND members.id_kelas ='$req->idkelas' AND members.nim_nip_user='$idusr'");
      // $datadosenkelas = DB::select("SELECT*FROM kelas,users WHERE kelas.id_dosen = users.nim_nip AND kelas.id ='$req->idkelas' ");
      // $datatugas = DB::select("SELECT*FROM tugas WHERE id='$idtugas'");
      // $tgltoday = date("l, d-m-Y: h:i:sa");
      //
      // $notif = "Funlearning Today ".$tgltoday;
      // foreach($datadosenkelas as $detail){
      //   foreach($datamemberusers as $dtmember){
      //     foreach($datatugas as $dttugas){
      //
      //       $data = array(
      //         'nama_user' => $dtmember->nama_user,
      //         'nama_mk' => $detail->nama_mk,
      //         'tipe' => "Tugas",
      //         'judul' => $dttugas->judul_tugas,
      //         'tanggalhariini' => $tgltoday,
      //       );
      //     }
      //
      //   }
      //
      //   Mail::send('mailnotifjawabtugas',$data, function ($mail) use($detail,$notif){
      //     $mail->to($detail->email, 'no-reply')
      //       ->subject("$notif");
      //     //$mail->from('funlearningitk@gmail.com','Funlearning ITK');
      //   });
