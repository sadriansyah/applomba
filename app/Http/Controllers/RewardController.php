...
class RewardController extends Controller
{
    ...
    function klaimreward(Request $req){
      ...
      if($maxklaim > 0 || $maxklaim <  0){
          ...
          if($mylevel > $minimumlevel){
            if($tipe == "tugas"){
              ...
              if(count($cek) > 0){
                ...
                $hadiahpoin = $poinreward;


                $maxklaim = $maxklaim-1;
                $updaterewardklaim = DB::select("UPDATE rewards SET batasan_klaim = '$maxklaim' WHERE id='$id'");
        ...
        $stat = "Reward Sudah Tidak Dapat diklaim lagi!";
      }

      $batasexp = 100 + ($mylevel * 100);
      $myexp  = $myexp+$hadiahpoin;
      $mypoin = $mypoin + $hadiahpoin;
      $poinawalmember = $poinawalmember + $hadiahpoin;
      while(true){
        if($myexp > $batasexp){
          $mylevel = $mylevel +1;
          $myexp = $myexp - $batasexp;
      ...
      $updateselfstatus = DB::select("UPDATE users SET level = '$mylevel', exp='$myexp', jumlah_poin ='$mypoin' WHERE nim_nip ='$nimnip'");
      $updateselfmember = DB::select("UPDATE members SET poin ='$poinawalmember' WHERE nim_nip_user='$nimnip' AND id_kelas ='$idkelas'");
    ...
    function buatreward(Request $req){
      ...
      //   Mail::send('mailnotifreward',$data, function ($mail) use($detail,$notif){
