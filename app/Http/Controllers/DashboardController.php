...
class DashboardController extends Controller
{
  ...
  public function nilairev(){

    $data['members'] = array();
    $members = DB::select("SELECT*, (SELECT nama_mk FROM kelas WHERE kelas.id=members.id_kelas) as nama_kelas, (SELECT level FROM users WHERE users.nim_nip = members.nim_nip_user) as level FROM members WHERE id_kelas='8' ");
    foreach($members as $member){
      //$tugas1= DB::select("SELECT nilai_tugas FROM jawabans  WHERE jawabans.id_user = members.nim_nip_user AND jawabans.id_tugas='25'");
      $jawaban = DB::table('jawabans')
                  ->where('id_tugas','25')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban1 = DB::table('jawabans')
                  ->where('id_tugas','26')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban2 = DB::table('jawabans')
                  ->where('id_tugas','30')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();

      $jawaban3 = DB::table('jawabans')
                  ->where('id_tugas','34')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban4 = DB::table('jawabans')
                  ->where('id_tugas','35')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban5 = DB::table('jawabans')
                  ->where('id_tugas','36')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban6 = DB::table('jawabans')
                  ->where('id_tugas','37')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban7 = DB::table('jawabans')
                  ->where('id_tugas','38')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban8 = DB::table('jawabans')
                  ->where('id_tugas','50')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban9 = DB::table('jawabans')
                  ->where('id_tugas','51')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();
      $jawaban10 = DB::table('jawabans')
                  ->where('id_tugas','52')
                  ->where('id_user',$member->nim_nip_user)
                  ->first();

      $arraymember = array();
      $isimember = DB::select("SELECT*FROM members WHERE id_kelas = '8'");
      foreach($isimember as $m){
        $cuks['nim_nip_user'] = $m->nim_nip_user;
        $cuks['poin'] = $m->poin;
        array_push($arraymember, $cuks);
       }
       $key = array_column($arraymember,'poin');
       array_multisort($key, SORT_DESC, $arraymember);
       //dd($arraymember);


      $fill['id'] = $member->id;
      $fill['nim_nip_user'] = $member->nim_nip_user;
      $fill['nama_kelas'] = $member->nama_kelas;
      $fill['level'] = $member->level;
      $fill['leaderboard'] = array_search($member->nim_nip_user,array_column($arraymember,'nim_nip_user')) +1;
      $fill['poin'] = $member->poin;
      if($jawaban){
        $fill['nilai1'] = $jawaban->nilai_tugas;
      }else {
        $fill['nilai1'] = 0;
      }
      if($jawaban1){
        $fill['nilai2'] = $jawaban1->nilai_tugas;
      }else {
        $fill['nilai2'] = 0;
      }
      if($jawaban2){
        $fill['nilai3'] = $jawaban2->nilai_tugas;
      }else {
        $fill['nilai3'] = 0;
      }
      if($jawaban3){
        $fill['nilai4'] = $jawaban3->nilai_tugas;
      }else {
        $fill['nilai4'] = 0;
      }
      if($jawaban4){
        $fill['nilai5'] = $jawaban4->nilai_tugas;
      }else {
        $fill['nilai5'] = 0;
      }
      if($jawaban5){
        $fill['nilai6'] = $jawaban5->nilai_tugas;
      }else {
        $fill['nilai6'] = 0;
      }
      if($jawaban6){
        $fill['nilai7'] = $jawaban6->nilai_tugas;
      }else {
        $fill['nilai7'] = 0;
      }
      if($jawaban7){
        $fill['nilai8'] = $jawaban7->nilai_tugas;
      }else {
        $fill['nilai8'] = 0;
      }
      if($jawaban8){
        $fill['nilai9'] = $jawaban8->nilai_tugas;
      }else {
        $fill['nilai9'] = 0;
      }
      if($jawaban9){
        $fill['nilai10'] = $jawaban9->nilai_tugas;
      }else {
        $fill['nilai10'] = 0;
      }
      if($jawaban10){
        $fill['nilai11'] = $jawaban10->nilai_tugas;
      }else {
        $fill['nilai11'] = 0;
      }
      array_push($data['members'],$fill);
    }
